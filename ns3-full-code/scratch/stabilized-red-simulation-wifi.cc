...
//   auto itr = stats.begin ();
//   Time curTime = Now ();
//   std::ofstream thr ("red-vs-ared-wifi-delay.dat", std::ios::out | std::ios::app);
//   thr <<  curTime << " " << itr->second.delaySum.GetSeconds () / itr->second.rxPackets << std::endl;
//   Simulator::Schedule (Seconds (0.4), &TraceDelay, monitor);
// }
...
//   auto itr = stats.begin ();
//   Time curTime = Now ();
//   std::ofstream thr ("red-vs-ared-wifi-drop.dat", std::ios::out | std::ios::app);
//   thr <<  curTime << " " << itr->second.lostPackets << std::endl;
//   Simulator::Schedule (Seconds (0.4), &TraceDrop, monitor);
// }
...
TraceQueue (Ptr<QueueDisc> queue)
{
  ...
  std::ofstream q ("red-vs-ared-wifi-queue.dat", std::ios::out | std::ios::app);
  q << Simulator::Now ().GetSeconds () << " " << qSize << " " << bufferOccupancy << std::endl;
  Simulator::Schedule (Seconds (0.01), &TraceQueue, queue);
}
// {
//   // uint32_t qSize = dev->GetQueue ()->GetNBytes ();
//   // std::ofstream q ("red-vs-ared-wifi-queue.dat", std::ios::out | std::ios::app);
//   // q << Simulator::Now ().GetSeconds () << " " << qSize << std::endl;
//   // Simulator::Schedule (Seconds (0.2), &TraceQueue, dev);
// }
...
int main (int argc, char *argv[])
{
  ...
  if (!modeBytes)
    ...
    // Config::SetDefault ("ns3::RedQueueDisc::PacketFilterList", DoubleValue (minTh));
  ...
  Simulator::Schedule (Seconds (0 + 0.000001), &TraceQueue, queueDiscRight.Get (0));
  ...
  if (stRight.GetNDroppedPackets (QueueDisc::INTERNAL_QUEUE_DROP) != 0)
    {
      std::cout << "There should be zero drops due to queue full (RIGHT)" << std::endl;
      exit (1);
    }
