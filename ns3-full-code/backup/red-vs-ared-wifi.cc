...
int main (int argc, char *argv[])
{
  ...
  phyLeft.Set("ChannelNumber",UintegerValue(6));
  phyRight.SetChannel (channelRight.Create ()); //  all the PHY layer objects created by the YansWifiPhyHelper share the same underlying channel
  phyRight.Set("ChannelNumber",UintegerValue(7));
  // MAC layer
  WifiHelper wifi;
  ...
  Ssid ssidRight = Ssid ("ns-3-ssid-right"); //  creates an 802.11 service set identifier (SSID) object
  ...
  apDevicesRight = wifi.Install (phyRight, macRight, wifiApNodeRight); // single AP which shares the same set of PHY-level Attributes (and channel) as the station
  ...
  // queueDiscs = tchBottleneck.Install (p2pBottleNeckDevices);
  queueDiscLeft = tchBottleneck.Install (wifiApNodeLeft.Get(0)->GetDevice(0));
  ...
  Simulator::Stop (Seconds (10.0)); // force stop, as otherwise AP node will generate beacon forever
  std::cout << "Running the simulation : Red vs ared wifi" << std::endl;
  Simulator::Run ();

  QueueDisc::Stats st = queueDiscRight.Get (0)->GetStats ();
